...
Character::Character(CImageData* pPlayerImgData, float startX, float startY)
    : m_health(100), m_speed(5), m_verticalVelocity(0.0f), 
...
void Character::InitCollider(int x, int y, int width, int height)
{
    ...
    Vector2 pos = GetTransform().position;
    m_pCollider = new AABBCollider(x, y, width, height);
}
...
void Character::InitAnimation()
{
    if (!m_spriteManager)
    {
        DebugManager::GetInstance().AddMessage(L"SpriteManager is not initialized in Character::InitAnimation()", 20.0f);
    }
    ...
    int currentFrame = 0;
    // Idle : 6 frames
    sequences.push_back({ CharacterAnim::AnimState::Idle, currentFrame, (currentFrame + 6 - 1), 0.15f * SECONDS_TO_MS });
    currentFrame += 6;

    // Run : 8 frames
    sequences.push_back({ CharacterAnim::AnimState::Run, currentFrame, (currentFrame + 8 - 1), 0.1f * SECONDS_TO_MS });
    currentFrame += 8;

    // 2 Combo Attack : 12 frames
    sequences.push_back({ CharacterAnim::AnimState::Attack, currentFrame, (currentFrame + 12 - 1), 0.08f * SECONDS_TO_MS });
    currentFrame += 12;

    // Death : 11 frames
    sequences.push_back({ CharacterAnim::AnimState::Death, currentFrame, (currentFrame + 11 - 1), 0.2f * SECONDS_TO_MS });
    currentFrame += 11;

    // Hurt : 4 frames
    sequences.push_back({ CharacterAnim::AnimState::Hurt, currentFrame, (currentFrame + 4 - 1), 0.1f * SECONDS_TO_MS });
    currentFrame += 4;

    // Jump : 3 frames
    sequences.push_back({ CharacterAnim::AnimState::Jump, currentFrame, (currentFrame + 3 - 1), 0.12f * SECONDS_TO_MS });
    currentFrame += 3;

    // UpToFall : 2 frames
    sequences.push_back({ CharacterAnim::AnimState::UpToFall, currentFrame, (currentFrame + 2 - 1), 0.12f * SECONDS_TO_MS });
    currentFrame += 2;

    // Fall : 3 frames
    sequences.push_back({ CharacterAnim::AnimState::Fall, currentFrame, (currentFrame + 3 - 1), 0.15f * SECONDS_TO_MS });
    currentFrame += 3;

    // Edge Grab : 5 frames
    sequences.push_back({ CharacterAnim::AnimState::EdgeGrab, currentFrame, (currentFrame + 5 - 1), 0.1f * SECONDS_TO_MS });
    currentFrame += 5;

    // Edge Idle : 6 frames
    sequences.push_back({ CharacterAnim::AnimState::EdgeIdle, currentFrame, (currentFrame + 6 - 1), 0.1f * SECONDS_TO_MS });
    currentFrame += 6;

    // WallSlide : 3 frames
    sequences.push_back({ CharacterAnim::AnimState::WallSlide, currentFrame, (currentFrame + 3 - 1), 0.1f * SECONDS_TO_MS });
    currentFrame += 3;

    // Crouch : 6 frames
    sequences.push_back({ CharacterAnim::AnimState::Crouch, currentFrame, (currentFrame + 6 - 1), 0.1f * SECONDS_TO_MS });
    currentFrame += 6;

    // Dash : 7 frames
    sequences.push_back({ CharacterAnim::AnimState::Dash, currentFrame, (currentFrame + 7 - 1), 0.08f * SECONDS_TO_MS });
    currentFrame += 7;

    // Dash-Attack : 10 frames
    sequences.push_back({ CharacterAnim::AnimState::DashAttack, currentFrame, (currentFrame + 10 - 1), 0.08f * SECONDS_TO_MS });
    currentFrame += 10;

    // Slide : 5 frames
    sequences.push_back({ CharacterAnim::AnimState::Slide, currentFrame, (currentFrame + 5 - 1), 0.1f * SECONDS_TO_MS });
    currentFrame += 5;

    // Ladder-Grab : 8 frames
    sequences.push_back({ CharacterAnim::AnimState::LadderGrab, currentFrame, (currentFrame + 8 - 1), 0.1f * SECONDS_TO_MS });
    currentFrame += 8;
    ...
    // 초기 상태를 Idle로 설정
    m_anim->SetState(CharacterAnim::AnimState::Idle);
}
...
{
    // 현재 애니메이션 상태를 가져옴
    CharacterAnim::AnimState currentAnim = m_anim->GetState();

    switch (currentAnim)
    {
    case CharacterAnim::AnimState::Idle:
    {
        // 입력에 따라 Run 또는 Jump로 전환
        if (InputManager::GetInstance().IsKeyDown(VK_SPACE))
        {
            m_anim->SetState(CharacterAnim::AnimState::Jump);
        }
        else if (InputManager::GetInstance().IsKeyDown(VK_LEFT) ||
            InputManager::GetInstance().IsKeyDown(VK_RIGHT))
        {
            m_anim->SetState(CharacterAnim::AnimState::Run);
        }
        break;
    }

    case CharacterAnim::AnimState::Run:
    {
    ...
    case CharacterAnim::AnimState::Jump:
    {
        // 상승 후 m_verticalVelocity가 0을 넘어 (즉, 하강 시작)하면 Fall로 전환
        if (m_verticalVelocity > 0.0f)
        {
            m_anim->SetState(CharacterAnim::AnimState::Fall);
        }
        break;
    }

    case CharacterAnim::AnimState::Fall:
    {
